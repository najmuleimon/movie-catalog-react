...
const Crew = ({movieId}) => {
  const {movieCrew} = useCredits(movieId);

  const [itemOffset, setItemOffset] = useState(0);
  let itemsPerPage = 6;
  const endOffset = itemOffset + itemsPerPage;
  const currentItems = movieCrew.slice(itemOffset, endOffset);
  const pageCount = Math.ceil(movieCrew.length / itemsPerPage);

  const handlePageClick = (event) => {
    const newOffset = (event.selected * itemsPerPage) % movieCrew.length;
    setItemOffset(newOffset);
  };

  return (
    <div>
      <h4 className="text-base font-medium text-white border-b border-gray-700 mb-6">Crew</h4>
